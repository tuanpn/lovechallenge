package vn.sunnet.game.entities;
...
public class Entity implements Poolable {
	...
	public boolean visibleByCamera(OrthographicCamera camera) {
		return camera.frustum.pointInFrustum(getPosition())
				|| camera.frustum.pointInFrustum(getRightBottomPoint())
				|| camera.frustum.pointInFrustum(getLeftBottomPoint())
				|| camera.frustum.pointInFrustum(getRightTopPoint());
	}

	public void setPositionInMeters(float x, float y) {
		setPosition(x / PsychicsManager.WORLD_TO_BOX, y
	...
	public Vector2 getCenteredPositionInMeters() {
		return new Vector2(this.getCenterPositionX()
				* PsychicsManager.WORLD_TO_BOX, this.getCenterPositionY()
				* PsychicsManager.WORLD_TO_BOX);
	}

	public String toString() {
		return this.id + " " + this.position.x + "x" + this.position.y + " - "
