...
public class PsychicsManager {
	...
	public PsychicsManager(LevelManager levelManager) {
		...
		boxCamera.setToOrtho(false, camera.viewportWidth * WORLD_TO_BOX,
				camera.viewportHeight * WORLD_TO_BOX);
		boxCamera.update();
		updateRayCamera();
	...
	private void updateRayCamera() {
		...
		boxCamera.setToOrtho(false, camera.viewportWidth * WORLD_TO_BOX,
				camera.viewportHeight * WORLD_TO_BOX);
		boxCamera.position.set(camera.position.x * WORLD_TO_BOX,
				camera.position.y * WORLD_TO_BOX, camera.position.z);
		boxCamera.update();
	...
	public static void updateEntityByBody(Body b) {
		...
		if (e != null) {
			e.setCenterPosition(b.getPosition().x
					/ PsychicsManager.WORLD_TO_BOX, b.getPosition().y
	...
	public void update(float delta) {
		if (enabled) {
			...
			for (Body bi : bodies) {
				PsychicsManager.updateEntityByBody(bi);
			}
		}
	}
	...
	public void syncWithEntites() {
		...
		for (Body bi : bodies) {
			PsychicsManager.updateBodyByEntity(bi);
		}
	}
	...
	private static void updateBodyByEntity(Body b) {
		...
		if (e != null) {
			b.setTransform(e.getCenteredPositionInMeters().x,
					e.getCenteredPositionInMeters().y, e.getRotation()
